...
@app.post("/echo_token")
def echo_token(body: EchoTokenBody):
    """
    Алгоритм "Эхо": лист получил маркер, возвращает эхо с данными о нагрузке.
    Процесс для не-инициатора (лист): получил маркер, вернул эхо родителю.
    """
    load = len(tasks)
    return {"loads": {NODE_ID: load}, "status": "echo"}
