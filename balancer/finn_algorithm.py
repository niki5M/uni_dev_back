# -*- coding: utf-8 -*-
"""
Волновой алгоритм Финна для ориентированной сети произвольной топологии.

Адаптация под тему распределённой системы университета:
- каждый процесс соответствует узлу из CONFIG.TREE (master, group_A/B/C, worker_800X);
- сеть ориентированная: рёбра идут от parent к children;
- алгоритм Финна используется для глобального "согласования" состояния:
  все процессы узнают, какие узлы логически "предшествовали" их последнему событию.

Реализована дискретная синхронная модель: обмен сообщениями идёт раундами.
Сообщения передают пары множеств (Inc, NInc) как в описании алгоритма Финна.
"""
...
@dataclass
class FinnMessage:
    """Сообщение алгоритма Финна: множества Inc и NInc."""
...
class FinnProcess:
    """
    Один процесс алгоритма Финна.

    Поля:
    - pid: идентификатор узла (например, 'master', 'group_A', 'worker_8001');
    - incoming: список идентификаторов узлов, имеющих дуги в этот узел;
    - outgoing: список идентификаторов узлов, куда этот узел посылает сообщения;
    - inc: множество Inc(s) из описания алгоритма;
    - ninc: множество NInc(s);
    - inbox: очередь входящих сообщений данного раунда.
    """
    ...
    def _broadcast_if_changed(
        ...
        """
        Если Inc или NInc изменились, рассылаем новое сообщение всем соседям по выходу.
        """
    ...
    def process_round(self) -> Tuple[List[Tuple[str, FinnMessage]], bool]:
        ...
        - читаем все сообщения из inbox;
        - объединяем Inc и NInc;
        - если получены сообщения от всех соседей по входу, добавляем себя в NInc;
        - при изменении Inc или NInc рассылаем новые сообщения по исходящим рёбрам.

        Возвращает:
        - список пар (receiver_id, message), которые нужно отправить в этом раунде;
        - флаг changed, показывающий, изменились ли множества.
        """
        if not self.inbox:
            return [], False
...
def build_processes_from_tree() -> Dict[str, FinnProcess]:
    """
    Строит процессы алгоритма Финна по топологии TREE из config.py.

    Рёбра ориентированы от родителя к детям.
    incoming[u] — все узлы, из которых есть дуга в u.
    outgoing[u] — все дети u.
    """
...
def run_finn_algorithm(initiator: str = "master", max_rounds: int = 100) -> Dict[str, Dict[str, Set[str]]]:
    """
    Запускает волновой алгоритм Финна на топологии TREE.

    initiator: узел, с которого мы начинаем "волну" (по теме работы — мастер).
    max_rounds: предохранитель от бесконечного цикла при ошибках в топологии.

    Возвращает словарь:
      node_id -> {"Inc": Inc(node_id), "NInc": NInc(node_id)}

    Условие остановки:
    - во всей сети нет изменений Inc/NInc в очередном раунде, И
    - для каждого узла Inc(s) == NInc(s) (условие return(OK)).
    """
...
def demo_finn() -> None:
    """
    Простейшая демонстрация алгоритма Финна для отчёта.

    Можно запустить отдельно:
        python3 -m balancer.finn_algorithm
    """
